		...
		while ($action -eq "Y") {
			if (Test-Path $projectFileAbsPath) {
				...
				if ($Args[0] -eq $null) {   
					...
					Write-Host "Building $projectFileAbsPath"
					$collectionofArgs = @($projectFileAbsPath, "/target:Version", "/target:Clean", "/target:clean_folders", "/target:Build", "/p:Configuration=Release", "/property:outdir=$binaryDirectory", "/fl1", "/fl2", "/flp1:Verbosity=normal;LogFile=$logfilepath;Append=true", "/flp2:Verbosity=detailed;LogFile=$errorlogfilepath;Append=true;errorsonly")
					& $msbuild $collectionofArgs 
